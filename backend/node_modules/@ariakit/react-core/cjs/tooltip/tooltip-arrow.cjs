"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _RTNVWWNMcjs = require('../__chunks/RTNVWWNM.cjs');


var _HSSJQ74Ccjs = require('../__chunks/HSSJQ74C.cjs');
require('../__chunks/PLY2V46G.cjs');
require('../__chunks/KPH4LMZO.cjs');
require('../__chunks/ZVS6SBRR.cjs');
require('../__chunks/VF7H2WBX.cjs');
require('../__chunks/EJVJIL4S.cjs');
require('../__chunks/76LCMKUI.cjs');




var _6TI3NXX6cjs = require('../__chunks/6TI3NXX6.cjs');
require('../__chunks/L5A2ID7S.cjs');
require('../__chunks/FDRJDQ5Y.cjs');



var _7EQBAZ46cjs = require('../__chunks/7EQBAZ46.cjs');

// src/tooltip/tooltip-arrow.tsx
var _misc = require('@ariakit/core/utils/misc');
var TagName = "div";
var useTooltipArrow = _6TI3NXX6cjs.createHook.call(void 0, 
  function useTooltipArrow2(_a) {
    var _b = _a, { store, size = 16 } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, ["store", "size"]);
    const context = _RTNVWWNMcjs.useTooltipContext.call(void 0, );
    store = store || context;
    _misc.invariant.call(void 0, 
      store,
      process.env.NODE_ENV !== "production" && "TooltipArrow must be wrapped in a Tooltip component."
    );
    props = _HSSJQ74Ccjs.usePopoverArrow.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store, size }, props));
    return props;
  }
);
var TooltipArrow = _6TI3NXX6cjs.forwardRef.call(void 0, function TooltipArrow2(props) {
  const htmlProps = useTooltipArrow(props);
  return _6TI3NXX6cjs.createElement.call(void 0, TagName, htmlProps);
});



exports.TooltipArrow = TooltipArrow; exports.useTooltipArrow = useTooltipArrow;
