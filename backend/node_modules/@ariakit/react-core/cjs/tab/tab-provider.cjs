"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _GHWOWBSZcjs = require('../__chunks/GHWOWBSZ.cjs');


var _TOESUYYHcjs = require('../__chunks/TOESUYYH.cjs');
require('../__chunks/44ZWQTBB.cjs');
require('../__chunks/RVRXLUKN.cjs');
require('../__chunks/4EKRCHT6.cjs');
require('../__chunks/K3X5TQGK.cjs');
require('../__chunks/ZVS6SBRR.cjs');
require('../__chunks/VF7H2WBX.cjs');
require('../__chunks/EJVJIL4S.cjs');
require('../__chunks/CWQU4R75.cjs');
require('../__chunks/76LCMKUI.cjs');
require('../__chunks/FVVTEJ4K.cjs');
require('../__chunks/6TI3NXX6.cjs');
require('../__chunks/L5A2ID7S.cjs');
require('../__chunks/FDRJDQ5Y.cjs');
require('../__chunks/7EQBAZ46.cjs');

// src/tab/tab-provider.tsx
var _jsxruntime = require('react/jsx-runtime');
function TabProvider(props = {}) {
  const store = _GHWOWBSZcjs.useTabStore.call(void 0, props);
  return /* @__PURE__ */ _jsxruntime.jsx.call(void 0, _TOESUYYHcjs.TabContextProvider, { value: store, children: props.children });
}


exports.TabProvider = TabProvider;
